...
extern MY_HIGHP_OR_MEDIUMP number hqs_bloom_distance;
extern MY_HIGHP_OR_MEDIUMP number hqs_bloom_fac;
extern MY_HIGHP_OR_MEDIUMP number hqs_bloom_cutoff;
extern MY_HIGHP_OR_MEDIUMP number hqs_color_correction;
...
vec4 effect(vec4 color, Image tex, vec2 tc, vec2 pc)
{
    ...
    //Add in some noise
    MY_HIGHP_OR_MEDIUMP number x = random(vec3(floor(pc.x / 2.0), floor(pc.y / 2.0), time));
	rgb_result = (1.0-clamp( noise_fac, 0.0,1.0 ))*rgb_result + x * clamp( noise_fac, 0.0,1.0 );

    rgb_result -= vec3(0.52);
    rgb_result *= (hqs_color_correction + 1.0);
    rgb_result += vec3(0.5);
    ...
    if (hqs_bloom_fac > 0.00001){
        MY_HIGHP_OR_MEDIUMP float bloom_dist = 0.0015*hqs_bloom_distance;
        MY_HIGHP_OR_MEDIUMP vec4 samp;
        MY_HIGHP_OR_MEDIUMP float cutoff = hqs_bloom_cutoff;

        for (MY_HIGHP_OR_MEDIUMP float i = -hqs_bloom_distance; i <= hqs_bloom_distance; ++i) {
            samp = Texel( tex, tc + (bloom_dist/hqs_bloom_distance)*vec2(float(i), 0.0));
            ...
            col += min(min(samp.r,samp.g),samp.b) * (hqs_bloom_distance - float(abs(i)));
        }   

        col /= float(hqs_bloom_distance);
        col *= hqs_bloom_fac;
        col.a = final_col.a;
    }

    return (final_col+col)*mask;
}
